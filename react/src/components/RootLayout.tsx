import { Outlet, Link, useLocation, useNavigate, ScrollRestoration } from "react-router";
...
export function RootLayout() {
  ...
  const [stats, setStats] = useState({ favorites: 0, active_bids: 0 });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setStats({
          ...
          active_bids: data.stats.active_bids
        });
      } catch (error) {
        // Ignore errors (not logged in)
      }
    };
    ...
    const checkAuth = async () => {
        ...
        try {
            ...
            if (profile) {
                const name = profile.first_name ? `${profile.first_name} ${profile.last_name}` : profile.email.split('@')[0];
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <Link to="/notifications">
                <button className="relative p-2.5 rounded-xl hover:bg-gray-100 transition-colors group">
                  ...
                  <span className="absolute top-2.5 left-2.5 w-2.5 h-2.5 bg-accent border-2 border-white rounded-full"></span>
                </button>
              </Link>
